...
const ENGINE_POWER = 10000.0
@onready var left_wheels = [$VehicleWheel3D,$VehicleWheel3D2,
$VehicleWheel3D3,$VehicleWheel3D4,$VehicleWheel3D5, $VehicleWheel3D11]
@onready var right_wheels = [$VehicleWheel3D6,$VehicleWheel3D7,
$VehicleWheel3D8,$VehicleWheel3D9,$VehicleWheel3D10, $VehicleWheel3D12]
...
func _process(delta: float) -> void:
	...
	if steer != 0 and move_dir != 0:
		...
		for wheel in left_wheels:
			wheel.engine_force = ENGINE_POWER * move_dir * 2
		for wheel in right_wheels:
			wheel.engine_force = ENGINE_POWER * move_dir * 2
